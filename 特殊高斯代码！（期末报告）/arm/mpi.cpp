    ...
    if (rank == 0){
        ...
        do{
            ...
            for (int i = 0; i < pasNum; i++){
                ...
                else MPI_Send(&Pas[i], Num + 1, MPI_FLOAT, flag, 0, MPI_COMM_WORLD);
            }
            super(rank, num_proc);
            ...
            for (int i = 0; i < pasNum; i++){
                int flag = i % num_proc;
                if (flag == rank) continue;
                else MPI_Recv(&Pas[i], Num + 1, MPI_FLOAT, flag, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            //升格消元子，然后判断是否结束
            sign = 0;
            for (int i = 0; i < pasNum; i++){
                //找到第i个被消元行的首项
                int temp = Pas[i][Num];
                if (temp == -1) continue; //说明已经被升格为消元子
                //看这个首项对应的消元子是不是为空，若为空，则补齐
                if (Act[temp][Num] == 0){
                    //补齐消元子
                    for (int k = 0; k < Num; k++)
